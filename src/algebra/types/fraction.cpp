  ...
  shared_ptr<Expression> Fraction::_reduce(shared_ptr<Expression> numerator, shared_ptr<Expression> denominator, shared_ptr<Scope> scope) {
    ...
    if (numerator == "undefined" || denominator == "undefined") {
      ...
      if (numerator == "multiplication") {
        auto mult = dynamic_cast<expressions::Multiplication*>(numerator.get());
      ...
      if (denominator == "multiplication") {
        auto mult = dynamic_cast<expressions::Multiplication*>(denominator.get());
      ...
      if (denominator_rest.size() == 0) {
        return expressions::Multiplication::_reduce(numerator_rest, scope);
      }
      ...
      numerator = expressions::Multiplication::_reduce(numerator_rest, scope);
      denominator = expressions::Multiplication::_reduce(denominator_rest, scope);
      ...
      return Fraction::_reduce(expressions::Multiplication::_reduce({fraction_matrix[0][0], fraction_matrix[1][1]}, scope), expressions::Multiplication::_reduce({fraction_matrix[0][1], fraction_matrix[1][0]}, scope), scope);
      ...
      auto addition_elements = dynamic_cast<expressions::Addition*>(numerator.get())->elements();
      for (auto element : addition_elements) {
        new_addition_elements.push_back(Fraction::_reduce(element, denominator));
      }
      return expressions::Addition::_reduce(new_addition_elements);
    }
    ...
    if (numerator == "multiplicaton") {
      numerator_elements = dynamic_cast<expressions::Multiplication*>(numerator.get())->elements();
    } else {
      numerator_elements = {numerator};
    }

    if (denominator == "multiplicaton") {
      denominator_elements = dynamic_cast<expressions::Multiplication*>(denominator.get())->elements();
    } else {
...

    {
      ...
      for (auto a_it = numerator_elements.begin(); a_it < numerator_elements.end(); ++a_it) {
        ...
        if (*a_it == "exponentiation") {
          auto exp = dynamic_cast<expressions::Exponentiation*>((*a_it).get());
          base = exp->base();
        ...
        for (auto b_it = denominator_elements.begin(); b_it < denominator_elements.end(); ++b_it) {
          if (*b_it == "exponentiation") {
            auto exp = dynamic_cast<expressions::Exponentiation*>((*b_it).get());
            if (exp->base() == base) {
              exponent = expressions::Addition::_reduce({exponent, expressions::Multiplication::_reduce({Integer::construct(-1), exp->exponent()}, scope)}, scope);
              denominator_elements.erase(b_it--);
            }
          } else if (*b_it == base) {
            exponent = expressions::Addition::_reduce({exponent, Integer::construct(-1)}, scope);
            denominator_elements.erase(b_it--);
          }
        }
        reduced_elements.push_back(expressions::Exponentiation::_reduce(base, exponent, scope));
    ...
    if (denominator_elements.size() == 0) {
      return expressions::Multiplication::_reduce(numerator_elements, scope);
    }
    ...
    numerator = expressions::Multiplication::_reduce(numerator_elements, scope);
    denominator = expressions::Multiplication::_reduce(denominator_elements, scope);
