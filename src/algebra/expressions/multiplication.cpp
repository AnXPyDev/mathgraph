...
namespace mathgraph::algebra::expressions {
  ...
  shared_ptr<Expression> Multiplication::_reduce(vector<shared_ptr<Expression>> elements, shared_ptr<Scope> scope) {
    ...
    shared_ptr<Expression> known_product = types::Integer::construct(1);
    {
...

    {
      ...
      for (auto a_it = elements.begin(); a_it < elements.end(); ++a_it) {
        ...
        if (*a_it == "exponentiation") {
          ...
          base = *a_it;
          exponent = types::Integer::construct(1);
        }
        for (auto b_it = elements.begin(); b_it < elements.end(); ++b_it) {
          ...
          if (*b_it == "exponentiation") {
            ...
            exponent = Addition::_reduce({exponent, types::Integer::construct(1)}, scope);
            elements.erase(b_it--);
          }
        }
    ...
    if (!(known_product == types::Integer::construct(1)) || elements.size() == 0) {
    ...
    if (reduce_as_fraction) {
      ...
      return types::Fraction::_reduce(m_numerator, m_denominator);
    }
  ...
  shared_ptr<Expression> Multiplication::_evaluate(vector<shared_ptr<Expression>> elements, shared_ptr<Scope> scope) {
    shared_ptr<Expression> result = types::Integer::construct(1);
    for (auto element : elements) {
