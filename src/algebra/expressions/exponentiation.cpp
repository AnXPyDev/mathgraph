...
namespace mathgraph::algebra::expressions {
  ...
  shared_ptr<Expression> Exponentiation::_reduce(shared_ptr<Expression> base, shared_ptr<Expression> exponent, shared_ptr<Scope> scope) {
    ...
    if (base == "list") {
      ...
      auto list_elements = dynamic_cast<types::List*>(base.get())->elements();
      ...
      vector<shared_ptr<Expression>> new_list_elements;
      auto list_elements = dynamic_cast<types::List*>(exponent.get())->elements();
      for (auto list_element : list_elements) {
        new_list_elements.push_back(Exponentiation::_reduce(base, list_element, scope));
      }
      return types::List::construct(new_list_elements);
    } else if (base == types::Integer::construct(1) || exponent == types::Integer::construct(1)) {
      return base;
    } else if (exponent == types::Integer::construct(0)) {
      return types::Integer::construct(1);
      ...
      auto frac = dynamic_cast<types::Fraction*>(base.get());
      return types::Fraction::_reduce(Exponentiation::_reduce(frac->numerator(), exponent, scope), Exponentiation::_reduce(frac->denominator(), exponent), scope);
    }
