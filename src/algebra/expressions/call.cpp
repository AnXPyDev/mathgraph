...
#include "algebra/expressions/assignment.hpp"
#include "algebra/expressions/call.hpp"
...
namespace mathgraph::algebra::expressions {
  ...
  shared_ptr<Expression> Call::_evaluate(shared_ptr<Expression> expression, shared_ptr<Expression> arguments, shared_ptr<Scope> scope) {
    if (arguments == "undefined") {
      ...
      auto list_elements = dynamic_cast<types::List*>(expression.get())->elements();
    ...
    if (expression == "symbol") {
      ...
      if (refered_expr == "function") {
        argument_names = dynamic_cast<types::Function*>(refered_expr.get())->arguments();
      }
    }
