...
namespace mathgraph::algebra::expressions {
  ...
  shared_ptr<Expression> Assignment::_evaluate(shared_ptr<Expression> left, shared_ptr<Expression> right, shared_ptr<Scope> scope) {
    if (left == "list") {
      ...
      auto left_elements = dynamic_cast<types::List*>(left.get())->elements();
      vector<shared_ptr<Expression>> right_elements = {right};
      if (right == "list") {
        right_elements = dynamic_cast<types::List*>(right.get())->elements();
      }
      ...
      return types::List::construct(return_elements);
    }
